...
class SpacecateController extends BaseController {
	...
	**/
    public function lists(){
        ...
        $count=M('spacecate')->where($map)->count();
        $Page = new \Think\Page($count,20);
        $show = $Page->show();
        $list = M('spacecate')->where($map)->order('id')->limit($Page->firstRow.','.$Page->listRows)->
        select();
        //获取空间父级标题
        foreach($list as $k=>$v){
            if($v['pid']==0)  {
                ...
                $fid=M('spacecate')->field('title')->where('id ='.$v['pid'])->find();
        ...
        if(!empty($search['pid']) && $search['is_two']==1){
            //获取一级分类id的父id
            $resultc=M('spacecate')->where('pid = 0')->find($search['pid']);
            $resultc2=M('spacecate')->where('pid = 0')->select();
            ...
            //获取二级分类,状态保持
            $result=M('spacecate')->where('pid = '.$resultc['pid'])->select();
            $cate2="";
            foreach($result as $v){
                if($v['id'] ==$search['pid']){
            ...
            $result=M('spacecate')->where('pid = 0')->select();
            if($search['pid']==1){
...
//            dump($_POST);exit();
            $spacecate = M("spacecate");
            $spacecate->create();
            $title=I('post.title');
            // $keywords=I('post.keywords');
            // $description=I('post.description');
            if(empty($title)){
                $this->error('请输入空间分类标题');
            }
            // if(empty($keywords)){
            //     $this->error('请输入空间分类关键字');
            // }
            // if(empty($description)){
            //     $this->error('请输入空间分类描述');
            // }

            $spacecate->addtime=time();
            $result = $spacecate->add();
            if($result){
                $this->success('添加成功',U("spacecate/lists"));
            }else{
    ...
    public function edit(){
        if(!IS_POST){
            $id=intval($_GET['id']);
            $cated=M('spacecate')->find($id);
            $catetop=M('spacecate')->find($cated['pid']);
            if(empty($cated)){
                $this->error('参数错误');
            }
            if($cated['pid']==0){
                ...
                //获取一级分类id的父id
                $resultc=M('spacecate')->where('pid = 0')->find($id);
                $resultc2=M('spacecate')->where('pid = 0')->select();
                $cate="<option value='0'>顶级分类</option>";
                foreach($resultc2 as $v){
                    if($v['id'] ==$resultc['pid']){
                ...
                //获取一级分类id的父id
                $resultc=M('spacecate')->where('pid = 0')->find($catetop['pid']);
                $resultc2=M('spacecate')->where('pid = 0')->select();
                $cate="<option value='0'>顶级分类</option>";
                foreach($resultc2 as $v){
                    if($v['id'] ==$resultc['id']){
                ...
                //获取二级分类,状态保持
                //$result=M('spacecate')->where('pid = '.$catetop['pid'])->select();
                // $cate2="<option value='{$resultc['pid']}'>顶级分类</option>";
                // foreach($result as $v){
                //     if($v['id'] ==$catetop['id']){
                //         $cate2 .= "<option value='{$v['id']}' selected>{$v['title']}</option>";
           ...
           // $this->assign('cate2',$cate2);//二级分类
            ...
            //     $this->error('请输入空间分类描述');
            // }
            $result = $spacecate->save();
            if($result !== false){
                $url = U("spacecate/lists");
