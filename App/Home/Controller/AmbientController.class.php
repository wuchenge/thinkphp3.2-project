...
class AmbientController extends BaseController {
    ...
    public function index(){
		...
		$Page  = new \Think\Hpage($count,8);// 实例化分页类 传入总记录数和每页显示的记录数
		$show  = $Page->show();// 分页显示输出
		//二级分类
		$cate=M('productcate')->where('pid = 59')->select();
		foreach ($cate as  $k=>$v) {
			//三级分类
		...
		$top='办公环境维护';
		$banner=M('banner')->where('type=2')->select();
