...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 空间分类管理 <span class="c-gray en">&gt;</span> 空间分类列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
<!-- 		<span class="select-box inline">
			...
			<input type="text" name="title" id="title" value="<?php echo ($search["title"]); ?>" placeholder=" 空间分类标题" style="width:250px" class="input-text">
			<button name="subgl" id="subgl" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜空间分类
			</button>
		</form>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
		<!--<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>-->
		<a class="btn btn-primary radius" data-title="添加空间分类"  onclick="add()" ><i class="Hui-iconfont">&#xe600;</i> 添加空间分类</a></span> <span class="r">共有数据：<strong><?php echo ($count); ?></strong> 条</span> </div>
	<div class="mt-20">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td class="f-14 td-manage">
						...
						<a style="text-decoration:none" class="ml-5"
												  href="<?php echo U('Spacecate/edit?id='.$vo['id']);?>" title="编辑"><i
...
<script type="text/javascript">
	//打开页面没有层级头
	function add(){
		location.href="<?php echo U('Spacecate/add');?>";
	}

/*资讯-删除*/
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		var url = "<?php echo U('Spacecate/del');?>";
...
//		$(obj).parents("tr").remove();
	...
	function cate(){
		...
		var url = "<?php echo U('Spacecate/cate1');?>";
