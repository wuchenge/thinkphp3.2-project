...
//        dump($search);
        ...
        $count=M('brand')->where($map)->count();
        $Page = adminpage($count,10);
        //$Page = new \Think\Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数(25)
        $show = $Page->show();// 分页显示输出
        // 进行分页数据查询 注意limit方法的参数要使用Page类的属性
        $list = M('brand')->where($map)->order('addtime desc')->limit($Page->firstRow.','.$Page->listRows)->
        select();
	...
	**/
    public function add(){
        if(!IS_POST){
            if(!empty($_GET['id'])){
               $newsxiugl=M('brand')->find($_GET['id']);
            }
            ...
            $result = $news->add();
            if($result){
                $this->success('新增品牌策划成功',U('Brand/lists'));
            }else{
                $this->error('新增品牌策划失败，请稍候操作');
            }
        }
    }
    ...
    public function edit(){
        if(!IS_POST){
            ...
            $newsxiugl = M('brand')->where("id = ".$_GET['id'])->find();
           ...
           if(!empty($imgurlglgo)){
            ...
            $result = $news->save();
            if($result !== false){
                $this->success('品牌策划修改成功',U("Brand/lists"));
            }else{
                $this->error('品牌策划修改失败，请稍候操作');
            }
        }
    }
    /**
