			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td class="f-14 td-manage">
						...
						<a style="text-decoration:none" class="ml-5"
												  href="<?php echo U('jifen/edit?id='.$vo['id']);?>" title="编辑"><i
							class="Hui-iconfont">&#xe6df;</i></a>
						<a style="text-decoration:none" onClick="article_del(this,'<?php echo ($vo["id"]); ?>')" href="javascript:;"
...
//打开页面没有层级头
	function add(){
		location.href="<?php echo U('jifen/add');?>";
	}
/*资讯-添加*/
...
function article_stop(obj,id){
	layer.confirm('确认要下架吗？',function(index){
		var url = "<?php echo U('jifen/xiaglgood');?>";
		$.post(url, {id: id}, function (data) {
			if (data.code == 1) {
...
function article_start(obj,id){
	layer.confirm('确认要上架吗？',function(index){
			var url = "<?php echo U('jifen/upglgood');?>";
			$.post(url, {id: id}, function (data) {
				if (data.code == 1) {
